/***************************************************************************************
 * Copyright (c) 2014-2024 Zihao Yu, Nanjing University
 *
 * NEMU is licensed under Mulan PSL v2.
 * You can use this software according to the terms and conditions of the Mulan PSL v2.
 * You may obtain a copy of Mulan PSL v2 at:
 *          http://license.coscl.org.cn/MulanPSL2
 *
 * THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND,
 * EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT,
 * MERCHANTABILITY OR FIT FOR A PARTICULAR PURPOSE.
 *
 * See the Mulan PSL v2 for more details.
 ***************************************************************************************/

#include "local-include/reg.h"
#include <cpu/cpu.h>
#include <cpu/decode.h>
#include <cpu/ifetch.h>
#include <cpu/ftrace.h>
#include <math.h>

#define R(i) gpr(i)
#define CSR(i) csr(i)
#define Mr vaddr_read
#define Mw vaddr_write

static int executed_inst = 0;

#ifdef CONFIG_FTRACE
int ftrace_len = 0;
Ftrace ftrace[MAX_FTRACE_LEN];
#endif

enum
{
    TYPE_I,
    TYPE_U,
    TYPE_S,
    TYPE_J,
    TYPE_B,
    TYPE_N, // none
};

#define src1R()                                                                                                        \
    do                                                                                                                 \
    {                                                                                                                  \
        *src1 = R(rs1);                                                                                                \
    } while (0)
#define src2R()                                                                                                        \
    do                                                                                                                 \
    {                                                                                                                  \
        *src2 = R(rs2);                                                                                                \
    } while (0)
#define immI()                                                                                                         \
    do                                                                                                                 \
    {                                                                                                                  \
        *imm = SEXT(BITS(i, 31, 20), 12);                                                                              \
    } while (0)
#define immU()                                                                                                         \
    do                                                                                                                 \
    {                                                                                                                  \
        *imm = SEXT(BITS(i, 31, 12), 20) << 12;                                                                        \
    } while (0)
#define immJ()                                                                                                         \
    do                                                                                                                 \
    {                                                                                                                  \
        *imm =                                                                                                         \
            ((SEXT(BITS(i, 31, 31), 1) << 19) | (BITS(i, 19, 12) << 11) | (BITS(i, 20, 20) << 10) | BITS(i, 30, 21))   \
            << 1;                                                                                                      \
    } while (0)
#define immB()                                                                                                         \
    do                                                                                                                 \
    {                                                                                                                  \
        *imm = ((SEXT(BITS(i, 31, 31), 1) << 11) | (BITS(i, 7, 7) << 10) | (BITS(i, 30, 25) << 4) | BITS(i, 11, 8))    \
               << 1;                                                                                                   \
    } while (0)
#define immS()                                                                                                         \
    do                                                                                                                 \
    {                                                                                                                  \
        *imm = (SEXT(BITS(i, 31, 25), 7) << 5) | BITS(i, 11, 7);                                                       \
    } while (0)

static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, word_t *shamt, int type)
{
    uint32_t i = s->isa.inst;
    int rs1 = BITS(i, 19, 15);
    int rs2 = BITS(i, 24, 20);
    *rd = BITS(i, 11, 7);
    switch (type)
    {
    case TYPE_I:
        src1R();
        immI();
        *shamt = rs2;
        break;
    case TYPE_U:
        immU();
        break;
    case TYPE_S:
        src1R();
        src2R();
        immS();
        break;
    case TYPE_J:
        immJ();
        break;
    case TYPE_B:
        immB();
        src1R();
        src2R();
        break;
    case TYPE_N:
        break;
    default:
        panic("unsupported type = %d", type);
    }
}

#ifdef CONFIG_FTRACE
static void record_ftrace2(Decode *s, word_t rd)
{
    Ftrace *ftr = &ftrace[ftrace_len];
    for (int i = 0; i < func_num; i++)
    {
        if (func_table[i].address == s->dnpc)
        {
            sprintf(ftr->func_name, "%s", func_table[i].func_name);
            ftr->type = CALL;
            ftr->func_addr = s->dnpc;
            ftr->inst_addr = s->pc;
            ftr->ret_addr = R(rd);
        }
    }
    if (ftr->type != CALL)
    {
        ftr->type = CALL_RET;
        ftr->inst_addr = s->pc;
        ftr->func_addr = s->dnpc;
        ftr->ret_addr = 0;
    }
    ftrace_len++;
    return;
}
#endif
#ifndef CONFIG_FTRACE 
static void record_ftrace2(Decode *s, word_t rd)
{
    return ;
}
#endif

static int decode_exec(Decode *s)
{
    s->dnpc = s->snpc;

#define INSTPAT_INST(s) ((s)->isa.inst)
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                                                           \
    {                                                                                                                  \
        int rd = 0;                                                                                                    \
        executed_inst++;                                                                                               \
        word_t src1 = 0, src2 = 0, imm = 0, shamt = 0;                                                                            \
        decode_operand(s, &rd, &src1, &src2, &imm, &shamt, concat(TYPE_, type));                                               \
        __VA_ARGS__;                                                                                                   \
    }

    INSTPAT_START();
    INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc, U, R(rd) = s->pc + imm);
    INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu, I, R(rd) = Mr(src1 + imm, 1));
    INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb, S, Mw(src1 + imm, 1, src2));
    INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh, S, Mw(src1 + imm, 2, src2));
    INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw, S, Mw(src1 + imm, 4, src2));

    INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi, I, R(rd) = src1 + imm);
    INSTPAT("??????? ????? ????? 100 ????? 00100 11", xor, I, R(rd) = src1 ^ imm);
    INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori, I, R(rd) = src1 | imm);
    INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli, I, R(rd) = src1 << (uint32_t)shamt);
    INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli, I, R(rd) = src1 >> (uint32_t)shamt);
    INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai, I, R(rd) = (int32_t)src1 >> shamt);
    INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu, I, R(rd) = src1 < imm);
    INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi, I, R(rd) = src1 & imm);

    INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui, U, R(rd) = imm);
    INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, R(rd) = s->pc + 4, s->dnpc = s->pc + imm, record_ftrace2(s, rd));
    INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I, R(rd) = s->pc + 4, s->dnpc = (src1 + imm) & ~1, record_ftrace2(s, rd));

    INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb, I, R(rd) = (int32_t)(int8_t)Mr(src1 + imm, 1));
    INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh, I, R(rd) = (int32_t)(int16_t)Mr(src1 + imm, 2));
    INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw, I, R(rd) = (int32_t)Mr(src1 + imm, 4));
    INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu, I, R(rd) = (uint8_t)Mr(src1 + imm, 1));
    INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu, I, R(rd) = (uint16_t)Mr(src1 + imm, 2));

    INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge, B,
            if ((int32_t)src1 >= (int32_t)src2) s->dnpc = s->pc + imm;);
    INSTPAT("??????? ????? ????? 111 ????? 11000 11", bge, B,
            if (src1 >= src2) s->dnpc = s->pc + imm;);
    INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne, B,
            if ((int32_t)src1 != (int32_t)src2) s->dnpc = s->pc + imm;);
    INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq, B,
            if ((int32_t)src1 == (int32_t)src2) s->dnpc = s->pc + imm;);
    INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu, B, if (src1 < src2) s->dnpc = s->pc + imm;);
    INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt, B,
            if ((int32_t)src1 < (int32_t)src2) s->dnpc = s->pc + imm;);

    INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub, S, R(rd) = src1 - src2;);
    INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add, S, R(rd) = src1 + src2;);
    INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and, S, R(rd) = src1 & src2;);
    INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem, B, R(rd) = (int32_t)src1 % (int32_t)src2);
    INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu, B, R(rd) = src1 % src2);
    INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or, S, R(rd) = src1 | src2);
    INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor, S, R(rd) = src1 ^ src2);
    INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll, S, R(rd) = src1 << src2;);
    INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl, S, R(rd) = src1 >> src2);
    INSTPAT("0100000 ????? ????? 101 ????? 01100 11", srl, S, R(rd) = (int32_t)src1 >> src2);
    INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt, S, R(rd) = (int32_t)src1 < (int32_t)src2);
    INSTPAT("0000000 ????? ????? 011 ????? 01100 11", slt, S, R(rd) = src1 < src2);
    INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul, S, R(rd) = (int32_t)src1 * (int32_t)src2);
    INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, S, R(rd) = (int64_t)(int32_t)src1 * (int64_t)(int32_t)src2 >> 32);
    INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, S, R(rd) = (uint64_t)src1 * (uint64_t)src2 >> 32);
    INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div, S, R(rd) = (int32_t)src1 / (int32_t)src2);
    INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu, S, R(rd) = src1 / src2);

    INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak, N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
    INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, N, RAISEINTR(s->pc));
    INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrww, I, word_t t = CSR(imm); CSR(imm) = src1; R(rd) = t;);
    INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs, I, word_t t = CSR(imm); CSR(imm) = t | src1; R(rd) = t;);
    INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc, I, word_t t = CSR(imm); CSR(imm) = t & (~src1); R(rd) = t;);
    INSTPAT("??????? ????? ????? ??? ????? ????? ??", inv, N, INV(s->pc));
    INSTPAT_END();

    R(0) = 0; // reset $zero to 0

    return 0;
}

int isa_exec_once(Decode *s)
{
    s->isa.inst = inst_fetch(&s->snpc, 4);
    return decode_exec(s);
}
