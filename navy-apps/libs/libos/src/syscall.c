...
{
    intptr_t *pre = heap_end;
    intptr_t *addr = heap_end;
    addr += increment;
    intptr_t ret = _syscall_(SYS_brk, (intptr_t)addr, 0, 0);
    heap_end = addr;
    return (void*)pre;
}
...
{
    int ret = _syscall_(SYS_time, (intptr_t)tv, (intptr_t)tz, 0);
    return ret;
}

int _execve(const char *fname, char *const argv[], char *const envp[])
{
    int ret = _syscall_(SYS_exit, (intptr_t)fname, (intptr_t)argv, (intptr_t)envp);
    return ret;
}
