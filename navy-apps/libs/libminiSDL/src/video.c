...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect)
{
    ...
    if (srcrect == NULL)
    {
        ...
        for (int i = 0; i < h; i++)
        {
            for (int j = 0; j < w; j++)
            { 
                ((uint32_t *)(dst->pixels))[(i + y) * screen_w + j + x] = ((uint32_t *)(src->pixels))[i * w + j];
            }
        }
    }
    for (int i = 0; i < h; i++)
    {
        for (int j = 0; j < w; j++)
        {
            ((uint32_t *)(dst->pixels))[(i + y) * screen_w + j + x] = 
                ((uint32_t *)(src->pixels))[(i + src_y) * src_w + j + src_x];
        }
    }
}
...
SDL_Surface *SDL_CreateRGBSurfaceFrom(void *pixels, int width, int height, int depth, int pitch, uint32_t Rmask,
